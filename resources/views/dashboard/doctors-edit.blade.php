...
<div class="h-screen flex">
    ...
    <div id="main" class="w-full overflow-auto bg-gray-100 h-screen p-10">
        <h1 class="font-bold text-3xl">Redaguoti daktarą [{{$doctor->user->name}} {{$doctor->user->lastname}}]</h1>
        ...
        <div class="flex justify-center align-middle h-screen items-center">
            <div class="w-full m-auto mt-6 px-6 py-4 bg-white shadow-md overflow-hidden rounded-lg">
                <form method="POST" action="/dashboard/doctors/edit/{{$doctor->id}}" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-6">
                        ...
                        <select id="form-role" name="category_id"
                            ...
                            @foreach($categories as $category)
                                    <option value="{{$category->id}}" {{$category->id == $doctor->category->id ? 'selected' : ''}}>{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
{{--                    <div class="mb-6">--}}
{{--                        <label for="specialization" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Specializacija</label>--}}
{{--                        <textarea name="specialization" rows="2"--}}
{{--                                  class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"--}}
{{--                                  placeholder="Leave a comment...">{{old('specialization', $doctor->specialization)}}</textarea>--}}
{{--                    </div>--}}
{{--                    <div class="mb-6">--}}
{{--                        <label for="education" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Išsilavinimas</label>--}}
{{--                        <textarea name="education" rows="2"--}}
{{--                                  class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"--}}
{{--                                  placeholder="Leave a comment...">{{old('education', $doctor->education)}}</textarea>--}}
{{--                    </div>--}}
                    <div class="mb-6">
                        <label for="body" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Darbo patirtis</label>
                        <textarea name="body" rows="2"
                                  class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                  placeholder="Iveskite teksta">{{old('experience', $doctor->experience)}}</textarea>
                    </div>
                    <div class="mb-6">
