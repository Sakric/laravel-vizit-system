...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="container mx-auto p-5">
        ...
        <h1 class="text-3xl mt-4">Mūsų specialistai</h1>
        ...
        <div class="flex justify-center w-full mt-10">
            ...
            @foreach($categories as $category)
                    <li class="px-4 py-3 bg-gray-200 rounded-full text-xl cursor-pointer" data-filter="{{$category->name}}">{{$category->name}}</li>
                @endforeach
            </ul>
        </div>
        ...
        <div id="doctors" class="w-full flex flex-wrap mt-10 min-h-fit gap-8">
            ...
            <div class="w-80 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700" data-category="{{$doctor->category->name}}">
                ...
                <div class="p-5">
                    ...
                    <a href="/doctors/{{$doctor->id}}" class="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                        Peržiurėti
    ...
    container.onclick = function (event) {
        ...
        if (element.nodeName === 'LI') {
            ...
            Array.from(filter).forEach(e => {
                e.classList.remove('bg-[#35BCA3]')
                e.classList.remove('text-white')
            });
            element.classList.add('bg-[#35BCA3]')
